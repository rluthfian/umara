            ...
            <div class="swiper mySwiper">
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide">
                        <img src="{{assets_url('images/cms/beranda/')}}{{$value->photo}}" style="width:100%;object-fit:cover;" alt="">
                        <div class="my-3 position-absolute" style="  top: 50%;left: 50%;transform: translate(-50%, -50%);">
                            <button style="opacity:0.5" onclick="modalEditCarousel('{{$value->id_carousel}}')" class="btn btn-primary">Edit</button>
                            <button style="opacity:0.5" onclick="hapusCarousel('{{$value->id_carousel}}')" class="btn btn-danger">Delete</button>
                        </div>
                    </div>
                    @endforeach
                </div>
    ...
    <div class="pt-5 row d-flex justify-content-center">
        <div class="col-12 py-5 py-lg-0 px-lg-5 text-center">
            ...
            <div class="swiper swiperTestimonial">
                <div class="swiper-wrapper">
                    @foreach($our_partnership as $key => $value)
                    <div class="swiper-slide text-center d-flex justify-content-center" style="border-radius:20px">
                        <div class="p-lg-3 text-center swiper-img">
                            <img src="{{assets_url('images/cms/beranda/')}}{{$value->photo}}" class="img-fluid" alt="">
                            <div class="my-3">
                                <button style="opacity:0.5" onclick="modalEditOurPartnership('{{$value->id_partnership}}')" class="btn btn-primary">Edit</button>
                                <button style="opacity:0.5" onclick="hapusOurPartnership('{{$value->id_partnership}}')" class="btn btn-danger">Delete</button>
                            </div>
                        </div>
                    </div>
                    @endforeach
    ...
    <div class="pt-5 row d-flex justify-content-center">
        <div class="col-12">
            <div class="row text-center text-dark justify-content-center d-flex">
                @foreach($our_service as $key => $value)
                <div data-aos="fade-up" class="col-6 col-lg-4 p-1 p-lg-3">
                    <div class="card py-3" style="border-radius:15px">
                        <div class="card-body px-lg-4">
                            <img class="image-layanan img-fluid" src="{{assets_url('images/cms/beranda/')}}{{$value->photo}}"  alt="">
                            <h1 class="card-title montserrat-bold-text pt-3">{{$value->title}}</h1>
                            <p class="card-text pb-lg-3">{{$value->content}}</p>
                            <a href="#" class="text-white btn btn-ask-admin py-2">{{$value->title}}</a>
                        </div>
                    </div>
                    <div class="my-3">
                        <button style="opacity:0.5" onclick="modalEditOurService('{{$value->id_layanan}}')" class="btn btn-primary">Edit</button>
                        <button style="opacity:0.5" onclick="hapusOurService('{{$value->id_layanan}}')" class="btn btn-danger">Delete</button>
                    </div>
                </div>
                @endforeach
            </div>
...
<script>
    function addCarousel() {
        ...
        $.ajax({
            url: "{{ base_url('admin/beranda/addCarousel') }}",
            type: "POST",
            data: formData,
            dataType: 'json',
...
<script>
    function modalEditCarousel(id_carousel) {
        $.ajax({
            url: "{{ base_url('admin/beranda/editCarousel/') }}" + id_carousel,
            type: "GET",
            success: function(response) {
                ...
                if (photo) {
                    $("#existingPhotoCarousel").attr("src", "{{ assets_url('images/cms/beranda/') }}" + photo);
...
<script>
    function updateCarousel(id_carousel) {
        ...
        $.ajax({
            url: "{{ base_url('admin/beranda/updateCarousel/') }}" + id_carousel,
            type: "POST",
            data: formData,
            dataType: 'json',
            contentType: false, // Set contentType to false when using FormData
...
<script>
    function hapusCarousel(id_carousel) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ base_url('admin/beranda/deleteCarousel/') }}" + id_carousel,
                    type: "POST",
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success') {
...
<script>
    function addOurPartnership() {
        ...
        $.ajax({
            url: "{{ base_url('admin/beranda/addOurPartnership') }}",
            type: "POST",
            data: formData,
            dataType: 'json',
            contentType: false, // Set contentType to false when using FormData
...
<script>
    function modalEditOurPartnership(id_partnership) {
        $.ajax({
            url: "{{ base_url('admin/beranda/editOurPartnership/') }}" + id_partnership,
            type: "GET",
            success: function(response) {
                var responseData = JSON.parse(response);
                var title = responseData.title;
                ...
                // Display the existing photo and provide an option to change it
                if (photo) {
                    $("#existingPhotoOurPartnership").attr("src", "{{ assets_url('images/cms/beranda/') }}" + photo);
...
<script>
    function updateOurPartnership(id_partnership) {
        ...
        $.ajax({
            url: "{{ base_url('admin/beranda/updateOurPartnership/') }}" + id_partnership,
            type: "POST",
            data: formData,
            dataType: 'json',
            contentType: false, // Set contentType to false when using FormData
...
<script>
    function hapusOurPartnership(id_partnership) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ base_url('admin/beranda/deleteOurPartnership/') }}" + id_partnership,
                    type: "POST",
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success') {
...
<script>
    function addOurService() {
        ...
        $.ajax({
            url: "{{ base_url('admin/beranda/addOurService') }}",
            type: "POST",
            data: formData,
            dataType: 'json',
            contentType: false, // Set contentType to false when using FormData
...
<script>
    function modalEditOurService(id_layanan) {
        $.ajax({
            url: "{{ base_url('admin/beranda/editOurService/') }}" + id_layanan,
            type: "GET",
            success: function(response) {
                var responseData = JSON.parse(response);
                var title = responseData.title;
                ...
                // Display the existing photo and provide an option to change it
                if (photo) {
                    $("#existingPhotoOurService").attr("src", "{{ assets_url('images/cms/beranda/') }}" + photo);
...
<script>
    function updateOurService(id_layanan) {
        ...
        $.ajax({
            url: "{{ base_url('admin/beranda/updateOurService/') }}" + id_layanan,
            type: "POST",
            data: formData,
            dataType: 'json',
            contentType: false, // Set contentType to false when using FormData
...
<script>
    function hapusOurService(id_layanan) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ base_url('admin/beranda/deleteOurService/') }}" + id_layanan,
                    type: "POST",
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success') {
