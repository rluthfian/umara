...
<div class="container-fluid">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <div>
                                    <button class="btn btn-primary" onclick="modalEditDosen('{{$value->id}}')">Edit</button>
                                    <button class="btn btn-danger" onclick="hapusDosen('{{$value->id}}')">Delete</button>
                                </div>
                            </td>
                        </tr>
                        @endforeach
    ...
    <script>
        function addDosen() {
            ...
            $.ajax({
                url: "{{ base_url('admin/dosens/addDosen') }}",
                type: "POST",
    ...
    <script>
        function modalEditDosen(id) {
            $.ajax({
                url: "{{ base_url('admin/dosens/editDosen/') }}" + id,
                type: "GET",
                success: function(response) {
                    ...
                    if (foto) {
                        $("#existingPhoto").attr("src", "{{ assets_url('images/dosens/') }}" + foto);
    ...
    <script>
        function updateDosen(id) {
            ...
            $.ajax({
                url: "{{ base_url('admin/dosens/updateDosen/') }}" + id,
                type: "POST",
                data: formData,
                dataType: 'json',
                contentType: false, // Set contentType to false when using FormData
    ...
    <script>
        function hapusDosen(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ base_url('admin/dosens/deleteDosen/') }}" + id,
                        type: "POST",
                        dataType: 'json',
                        success: function(response) {
                            if (response.status === 'success') {
